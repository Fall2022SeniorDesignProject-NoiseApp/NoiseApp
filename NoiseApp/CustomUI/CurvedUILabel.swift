...
//  NoiseApp
//
//  Created by Jacob Blair on 10/21/22.
//
...
class UILabelX: UILabel {
    ...
    */
    func centre(text str: String, context: CGContext, radius r:CGFloat, angle theta: CGFloat, slantAngle: CGFloat) {
        ...
        let offset = str.size(withAttributes: attributes as [NSAttributedString.Key : Any])
        // Move the origin by half the size of the text
        context.translateBy(x: -offset.width / 2, y: -offset.height / 2) // Move the origin to the centre of the text (negating the y-axis manually)
        // Draw the text
        str.draw(at: CGPoint(x: 0, y: 0), withAttributes: attributes as [NSAttributedString.Key : Any])
        // Restore the context
    ...
    func getRadiusForLabel() -> CGFloat {
        ...
        let heightOfFont = self.text?.size(withAttributes: [NSAttributedString.Key.font: self.font ?? 17]).height ?? 0
